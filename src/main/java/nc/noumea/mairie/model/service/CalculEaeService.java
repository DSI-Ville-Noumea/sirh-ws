...
public class CalculEaeService implements ICalculEaeService {
	...
	public CalculEaeInfosDto getAffectationActiveByAgent(Integer idAgent, Integer anneeFormation) {
		...
		affectation.getFichePoste().getService().setDirection(
				siservSrv.getDirection(affectation.getFichePoste().getService().getServi()) == null ? "" : siservSrv.getDirection(
						affectation.getFichePoste().getService().getServi()).getLiServ());
		affectation.getFichePoste().getService().setSection(
				siservSrv.getSection(affectation.getFichePoste().getService().getServi()) == null ? "" : siservSrv.getSection(
						affectation.getFichePoste().getService().getServi()).getLiServ());
		...
		if(null != affectation.getFichePoste().getSuperieurHierarchique().getTitrePoste()) {
			...
			titrePosteResponable.setLibTitrePoste(affectation.getFichePoste().getSuperieurHierarchique().getTitrePoste().getLibTitrePoste());
		...
		if(null != affectation.getAgent()) {
			dto.setPositionAdmAgentEnCours(spadmnService.chercherPositionAdmAgentEnCours(affectation.getAgent().getNomatr()));
			dto.setPositionAdmAgentAncienne(spadmnService.chercherPositionAdmAgentAncienne(affectation.getAgent().getNomatr()));
			dto.setCarriereFonctionnaireAncienne(spCarrService.getCarriereFonctionnaireAncienne(affectation.getAgent().getNomatr()));
		...
		if(null != listSpmtsr) {
			for(Spmtsr spMtsr : listSpmtsr) {
				ParcoursProDto parcoursProDto = new ParcoursProDto(spMtsr);
					parcoursProDto.setDirection(siservSrv.getDirection(spMtsr.getServi()) == null ? "" : siservSrv.getDirection(spMtsr.getServi()).getLiServ());
					parcoursProDto.setService(siservSrv.getService(spMtsr.getServi()) == null ? "" : siservSrv.getService(spMtsr.getServi()).getLiServ());
	...
	public List<AgentDto> getListeAgentEligibleEAESansAffectes() {
		...
		List<java.math.BigDecimal> listNoMatr = mairieRepository.getListeCarriereActiveAvecPA();
		
		List<AgentDto> result = new ArrayList<AgentDto>();
		if(null != listNoMatr) {
			for(java.math.BigDecimal noMatr : listNoMatr) {
	...
	public List<AgentDto> getListeAgentEligibleEAEAffectes() {
		...
		List<java.math.BigDecimal> listNoMatr = mairieRepository.getListeCarriereActiveAvecPAAffecte();
